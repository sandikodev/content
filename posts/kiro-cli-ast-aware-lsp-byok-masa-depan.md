...
Satu baris kode di Kiro CLI ini membuat saya berpikir panjang:

```rust
let matches: Vec<_> = content.match_indices(old_str).collect();
```

Sederhana. Bekerja. Tapi juga — secara fundamental — tidak tahu apa-apa tentang kode yang sedang ia edit.
...
Kiro CLI menggunakan `match_indices` untuk menemukan string yang akan diganti dalam operasi `str_replace`. Pencarian dilakukan secara literal — byte per byte, karakter per karakter. Jika `old_str` muncul dua kali dalam file, operasi gagal. Jika ada whitespace yang berbeda, operasi gagal. Jika konteks semantiknya berbeda tapi teksnya sama, Kiro tidak bisa membedakan.
...
Pendekatan ini pragmatis dan cukup untuk banyak kasus. Tapi ada skenario yang sering muncul dalam praktik: dua fungsi dengan nama yang mirip, dua blok `if` dengan kondisi yang identik, atau dua `impl` block untuk trait yang sama. Dalam semua kasus itu, model harus memberikan konteks yang sangat spesifik agar `str_replace` tidak ambigu — beban yang seharusnya tidak perlu ditanggung oleh LLM.
...
AST-aware editing berarti kita tidak lagi mencari string `"fn calculate"` lalu menggantinya secara buta. Kita parse file menjadi AST, temukan node `FnItem` dengan nama `calculate`, lalu modifikasi node tersebut secara presisi — tanpa risiko mengenai kode lain yang kebetulan mengandung teks yang sama.
...
Untuk multi-language support, `tree-sitter` adalah pilihan yang lebih generalis — parser incremental yang mendukung puluhan bahasa (Rust, Python, TypeScript, Go, dan lainnya) dengan API yang konsisten:
...
Setelah setiap operasi edit, OpenCode menjalankan diagnostics check melalui LSP. Hasilnya — error, warning, type mismatch — dilaporkan kembali ke model sebagai konteks. Model bisa melihat bahwa edit yang baru saja dilakukan menyebabkan type error di baris 47, lalu melakukan koreksi tanpa perlu intervensi manusia.
...
LSP protocol bisa diimplementasikan dengan `tower-lsp`. Crate ini menyediakan framework untuk membangun LSP server maupun client. Untuk kebutuhan Kiro CLI — yaitu menjadi LSP client yang bisa query diagnostics dari language server yang sudah berjalan — ini adalah abstraksi yang tepat.
...
Ada satu hal lagi yang relevan langsung dengan AST-aware editing: byte-level precision. Tree-sitter mengembalikan posisi node dalam bentuk byte range, bukan karakter range. Rust's `&str` dan slice semantics membuat operasi ini natural dan aman — tidak ada off-by-one error yang tersembunyi di balik encoding.
...
Saat ini, Kiro CLI terikat pada model tertentu dengan konfigurasi yang terbatas. Tapi bayangkan skenario ini: komunitas open source bisa menghubungkan LLM mereka sendiri — model yang di-host sendiri, model fine-tuned, atau model dari provider alternatif — ke Kiro CLI melalui konfigurasi API endpoint yang sederhana.
...
Ini bukan wishlist abstrak. Ada tiga issue yang bisa dibuka di repo Kiro CLI dengan scope yang terdefinisi.

Issue pertama adalah integrasi `tree-sitter` untuk AST-aware `str_replace`. Scopenya: tambahkan optional AST-aware mode untuk `str_replace`. Jika file adalah Rust/Python/TypeScript dan `old_str` ambigu (multiple matches), gunakan tree-sitter untuk disambiguasi berdasarkan node type dan scope. Fallback ke behavior saat ini jika tree-sitter tidak bisa resolve.
...
Issue kedua adalah LSP diagnostics check setelah setiap `fs_write`. Setelah operasi `fs_write` atau `str_replace` berhasil, jalankan diagnostics check melalui LSP client. Jika ada error, tambahkan ke context sebagai `<diagnostics>` block sebelum model melanjutkan. Fitur ini opsional dan bisa di-disable via config.

Issue ketiga adalah BYOK API endpoint configuration. Tambahkan support untuk custom model endpoint di config file dengan format OpenAI-compatible API — yang sudah didukung oleh Ollama, vLLM, LM Studio, dan lainnya. Ini memungkinkan komunitas menggunakan model lokal atau model fine-tuned mereka sendiri.
...
*Artikel ini adalah bagian dari seri eksplorasi Kiro CLI dari perspektif Rust engineering. Jika tertarik berkontribusi pada salah satu proposal di atas, cek repo Kiro CLI dan buka diskusi di issue tracker.*

...
Ini bukan artikel tentang "Windows vs Linux". Ini tentang dua byte — `0x0D` dan `0x0A` — yang sudah menyebabkan bug selama lebih dari 50 tahun, dan masih terus menyebabkan bug hari ini, termasuk di dalam AI agent yang saya kerjakan.
...
Pada era mesin tik dan teletype (mesin cetak jarak jauh), ada dua operasi fisik yang berbeda untuk pindah ke baris baru:

- **Carriage Return (CR)**: menggerakkan "carriage" — kepala cetak — kembali ke posisi paling kiri. Ini operasi mekanis yang butuh waktu.
- **Line Feed (LF)**: menggulung kertas satu baris ke atas.

Kedua operasi ini harus dilakukan secara berurutan untuk memulai baris baru. Maka lahirlah `CR+LF` sebagai standar de facto untuk "pindah baris" di perangkat berbasis karakter.
...
Mari kita konkret. Ketika kamu menulis string `"hello\nworld"` di Unix, ini yang tersimpan di memory:
...
Ketika file yang sama dibuat di Windows, isinya:
...
Satu byte ekstra: `0x0D`. Tidak terlihat di text editor. Tidak terlihat di `cat`. Tapi ada di sana, diam-diam, menunggu untuk merusak asumsi kode kamu.

Di Rust, kamu bisa memverifikasi ini langsung:
...
Output yang kamu harapkan untuk baris `fn main() {`:

```
Line 3: 'fn main() {' (len=12)
```

Output yang sebenarnya kamu dapat:

```
Line 3: 'fn main() {' (len=13)
```

`trim()` memang menghapus `\r` dari tampilan, tapi `line.len()` menghitung panjang *sebelum* trim. Baris yang "terlihat" panjangnya 12 karakter sebenarnya 13 byte di memory.

### Kenapa Ini Berbahaya

Masalah ini menjadi kritis ketika kamu menghitung **byte offset** untuk operasi patch. Misalnya, kamu ingin mengganti teks di posisi byte tertentu:
...
Jika `content` mengandung `\r\n`, setiap baris akan dihitung 1 byte lebih panjang dari yang seharusnya (karena `\r` ikut terhitung). Setelah 100 baris, offset kamu sudah meleset 100 byte. Patch yang kamu terapkan akan mengenai lokasi yang salah.

Ini adalah **silent data corruption** — tidak ada error, tidak ada panic, tapi hasilnya salah.

### Kasus Fuzzy Matching

Bug ini semakin parah di konteks fuzzy matching. Ketika Kiro CLI mencoba mencocokkan potongan kode dari AI response dengan konten file aktual, perbandingan string seperti ini akan gagal:
...
Fuzzy matching yang tidak menormalisasi line ending akan melaporkan similarity score yang rendah untuk dua string yang secara semantik identik. AI agent kemudian bingung, mencoba strategi lain, dan akhirnya gagal menerapkan perubahan.
...
`Cow<str>` adalah singkatan dari **Clone-on-Write**. Ini adalah enum di Rust standard library:
...
Untuk kasus kita, `Cow<str>` bisa berupa:
- `Cow::Borrowed(&str)` — referensi ke string asli, tanpa alokasi baru
- `Cow::Owned(String)` — string baru yang dialokasikan di heap

Kenapa ini lebih efisien? Pertimbangkan skenario nyata: dari 1000 file yang diproses Kiro CLI, mungkin hanya 50 yang berasal dari Windows dan mengandung `\r\n`. Jika kita selalu mengembalikan `String`, kita melakukan 1000 alokasi heap. Dengan `Cow<str>`, kita hanya melakukan 50 alokasi — untuk file yang memang perlu dimodifikasi.
...
Setelah melakukan operasi (patch, replacement, dll.) pada string yang sudah dinormalisasi, kamu mungkin perlu mengembalikan line ending ke format aslinya sebelum menulis kembali ke file. Ini penting untuk menghormati konvensi proyek pengguna.
...
Format lama macOS (sebelum OS X) menggunakan `\r` saja sebagai line ending. Ini sangat jarang ditemui di 2026, tapi jika kamu memproses file legacy, perlu penanganan tambahan:
...
Git punya mekanisme konversi line ending otomatis via `core.autocrlf`. Di Windows, setting `true` akan mengkonversi LF ke CRLF saat checkout dan sebaliknya saat commit. Ini berarti file di working directory Windows bisa berbeda byte-per-byte dengan file di repository, meski konten "sama".

Untuk tool yang bekerja dengan file di working directory (seperti Kiro CLI), kamu tidak bisa berasumsi bahwa file sudah dalam format LF hanya karena repository menggunakan LF. Normalisasi tetap diperlukan.
...
Setiap kali data masuk ke sistem kamu dari sumber eksternal — file, network, user input — ada kemungkinan data itu tidak dalam format yang kamu asumsikan. Pilihan kamu ada dua:

1. **Defensive coding di setiap titik**: cek `\r` di setiap fungsi yang memproses string. Ini menyebar kompleksitas, mudah terlewat, dan sulit di-test.

2. **Normalisasi di boundary**: satu titik masuk, satu titik keluar. Logika internal bekerja dengan asumsi yang konsisten.

Pilihan kedua bukan hanya lebih bersih — ini adalah cara berpikir yang benar tentang **data contracts**. Fungsi internal kamu punya kontrak: "saya menerima string dengan LF". Siapa yang bertanggung jawab memenuhi kontrak itu? Boundary layer, bukan setiap fungsi di dalam.

Analogi yang tepat: ini seperti sanitasi input di web application. Kamu tidak meng-escape HTML di setiap fungsi yang menyentuh user input. Kamu melakukannya di satu tempat — saat data masuk atau saat data di-render. Prinsipnya sama.

---

## Kesimpulan

Dua byte. `0x0D` dan `0x0A`. Warisan dari mesin tik tahun 1960-an yang masih menyebabkan bug di AI agent tahun 2026.
...
Perbaikannya ada di tiga hal:

- Pahami representasi byte: `\r\n` adalah dua karakter, bukan satu
- Gunakan `Cow<str>` untuk normalisasi yang efisien — zero-copy jika tidak perlu
- Normalisasi di boundary, bukan di dalam logic
